...
const Hero = ({ onStartRecording }) => {
    ...
    const initProtocol = () => {
        ...
        const i = setInterval(() => {
            ...
            if (b >= 99) {
                clearInterval(i);
                // Instead of failing, let's trigger the actual recording!
                setTimeout(() => {
                    setNetState('IDLE');
                    setBuffer(0);
                    onStartRecording();
        ...
        <section className="relative h-screen flex items-center justify-center overflow-hidden bg-[url('https://images.unsplash.com/photo-1550745165-9bc0b252726f?q=80&w=2070&auto=format&fit=crop')] bg-cover bg-center">
            ...
            <div className="relative z-10 text-center px-6 max-w-5xl mt-20">
                <div className="inline-block border border-[#ccfa00] px-4 py-1 mb-8 bg-black/50 backdrop-blur">
                    <span className="text-[#ccfa00] font-mono animate-pulse">SYSTEM STATUS: READY FOR UPLINK</span>
                </div>
                ...
                >
                    <span className="text-transparent bg-clip-text bg-gradient-to-b from-white to-[#555]">Record</span><br />
                    <span className="text-[#ccfa00] neon-text">Share</span><br />
                    <span className="text-transparent bg-clip-text bg-gradient-to-b from-white to-[#555]">Instantly</span>
                </h1>

                <p className="text-xl md:text-2xl text-gray-300 font-mono tracking-widest max-w-2xl mx-auto mb-12">
                    THE ALGORITHM REQUIRES DATA. <br />
                    <span className="text-[#ccfa00]">CAPTURE YOUR SCREEN. RULE THE STREAM.</span>
                </p>
                ...
                <div className="flex flex-col md:flex-row gap-6 justify-center items-center">
                    ...
                    ${netState === 'SYNCING' ? 'bg-white text-black border-white' : 'bg-[#ccfa00] text-black hover:bg-white'}
                `}
                    >
                        {netState === 'IDLE' && "Initialize Protocol"}
                        {netState === 'SYNCING' && `Syncing Stream... ${buffer}%`}
                    </button>
            ...
            <div className="absolute bottom-0 w-full bg-[#ccfa00] overflow-hidden py-2 whitespace-nowrap">
                <div className="animate-[marquee_10s_linear_infinite] font-black text-black text-2xl italic inline-block">
